 ...
 */
  ...
  const initEssayFontControl = () => {
    const article = document.querySelector('#article-container[data-essay-reading="true"]')
    const slider = document.querySelector('[data-essay-font-size-slider]')
    const value = document.querySelector('[data-essay-font-size-value]')
    if (!article || !slider || !value) return

    const storageKey = 'essay-reading-font-scale'
    const min = Number(slider.min)
    const max = Number(slider.max)
    const fallback = Number(article.dataset.essayFontDefault || slider.value)

    const clampScale = scale => Math.min(max, Math.max(min, scale))
    const syncValue = scale => {
      const percent = `${Math.round(scale * 100)}%`
      slider.value = scale.toFixed(2)
      value.textContent = percent
      article.style.setProperty('--essay-reading-font-scale', scale.toFixed(2))
    }

    const savedScale = Number(btf.saveToLocal.get(storageKey))
    const currentScale = Number.isFinite(savedScale) ? clampScale(savedScale) : clampScale(fallback)
    syncValue(currentScale)

    if (slider.dataset.essayFontBound === 'true') return

    slider.addEventListener('input', event => {
      const nextScale = clampScale(Number(event.target.value))
      syncValue(nextScale)
      btf.saveToLocal.set(storageKey, nextScale.toFixed(2), 365)
    })

    slider.dataset.essayFontBound = 'true'
  }
